...
int main(void) {
    ...
    do {
        ...
        WowWorldResult result = tbc_client_opcode_read(&reader, &opcode);
    ...
    do {
        ...
        WowWorldResult result = tbc_server_opcode_read(&reader, &opcode);
    ...
    do {
        ...
        if (result != WWM_RESULT_SUCCESS) {
            printf(__FILE__ ":" STRINGIFY(__LINE__) " SMSG_MOTD 0 failed to read: '%s'\n", wwm_error_code_to_string(result));
            abort();

        }
        if (opcode.opcode != SMSG_MOTD) {
            printf(__FILE__ ":" STRINGIFY(__LINE__) " SMSG_MOTD 0 read wrong opcode: '0x%x' instead of '0x%x'\n", opcode.opcode, SMSG_MOTD);
            abort();
        ...
        /* C89 scope to allow variable declarations */ {
            ...
            if (result != WWM_RESULT_SUCCESS) {
                printf(__FILE__ ":" STRINGIFY(__LINE__) " SMSG_MOTD 0 failed to write: '%s'\n", wwm_error_code_to_string(result));
                abort();
