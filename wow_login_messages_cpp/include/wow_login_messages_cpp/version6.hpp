...
struct ClientOpcode {
    ...
    explicit ClientOpcode(Opcode op) : opcode(op) {
        if (opcode == Opcode::CMD_AUTH_LOGON_PROOF) {
            new (&this->CMD_AUTH_LOGON_PROOF) version6::CMD_AUTH_LOGON_PROOF_Client();
        }
        if (opcode == Opcode::CMD_AUTH_RECONNECT_PROOF) {
            new (&this->CMD_AUTH_RECONNECT_PROOF) version6::CMD_AUTH_RECONNECT_PROOF_Client();
        }
        if (opcode == Opcode::CMD_REALM_LIST) {
            new (&this->CMD_REALM_LIST) version6::CMD_REALM_LIST_Client();
        }
        if (opcode == Opcode::CMD_XFER_ACCEPT) {
            new (&this->CMD_XFER_ACCEPT) version6::CMD_XFER_ACCEPT();
        }
        if (opcode == Opcode::CMD_XFER_RESUME) {
            new (&this->CMD_XFER_RESUME) version6::CMD_XFER_RESUME();
        }
        if (opcode == Opcode::CMD_XFER_CANCEL) {
            new (&this->CMD_XFER_CANCEL) version6::CMD_XFER_CANCEL();
        }
    }
...
struct ServerOpcode {
    ...
    explicit ServerOpcode(Opcode op) : opcode(op) {
        if (opcode == Opcode::CMD_AUTH_LOGON_CHALLENGE) {
            new (&this->CMD_AUTH_LOGON_CHALLENGE) version6::CMD_AUTH_LOGON_CHALLENGE_Server();
        }
        if (opcode == Opcode::CMD_AUTH_LOGON_PROOF) {
            new (&this->CMD_AUTH_LOGON_PROOF) version6::CMD_AUTH_LOGON_PROOF_Server();
        }
        if (opcode == Opcode::CMD_AUTH_RECONNECT_CHALLENGE) {
            new (&this->CMD_AUTH_RECONNECT_CHALLENGE) version6::CMD_AUTH_RECONNECT_CHALLENGE_Server();
        }
        if (opcode == Opcode::CMD_AUTH_RECONNECT_PROOF) {
            new (&this->CMD_AUTH_RECONNECT_PROOF) version6::CMD_AUTH_RECONNECT_PROOF_Server();
        }
        if (opcode == Opcode::CMD_REALM_LIST) {
            new (&this->CMD_REALM_LIST) version6::CMD_REALM_LIST_Server();
        }
        if (opcode == Opcode::CMD_XFER_INITIATE) {
            new (&this->CMD_XFER_INITIATE) version6::CMD_XFER_INITIATE();
        }
        if (opcode == Opcode::CMD_XFER_DATA) {
            new (&this->CMD_XFER_DATA) version6::CMD_XFER_DATA();
        }
    }
    ...
    explicit ServerOpcode(version6::CMD_AUTH_LOGON_CHALLENGE_Server&& obj) {
        ...
        new (&this->CMD_AUTH_LOGON_CHALLENGE) version6::CMD_AUTH_LOGON_CHALLENGE_Server (obj);
    }
    explicit ServerOpcode(version6::CMD_AUTH_LOGON_PROOF_Server&& obj) {
        opcode = Opcode::CMD_AUTH_LOGON_PROOF;
        new (&this->CMD_AUTH_LOGON_PROOF) version6::CMD_AUTH_LOGON_PROOF_Server (obj);
    }
    explicit ServerOpcode(version6::CMD_AUTH_RECONNECT_CHALLENGE_Server&& obj) {
        opcode = Opcode::CMD_AUTH_RECONNECT_CHALLENGE;
        new (&this->CMD_AUTH_RECONNECT_CHALLENGE) version6::CMD_AUTH_RECONNECT_CHALLENGE_Server (obj);
    }
    explicit ServerOpcode(version6::CMD_AUTH_RECONNECT_PROOF_Server&& obj) {
        opcode = Opcode::CMD_AUTH_RECONNECT_PROOF;
        new (&this->CMD_AUTH_RECONNECT_PROOF) version6::CMD_AUTH_RECONNECT_PROOF_Server (obj);
    }
    explicit ServerOpcode(version6::CMD_REALM_LIST_Server&& obj) {
        opcode = Opcode::CMD_REALM_LIST;
        new (&this->CMD_REALM_LIST) version6::CMD_REALM_LIST_Server (obj);
    }
    explicit ServerOpcode(version6::CMD_XFER_INITIATE&& obj) {
        opcode = Opcode::CMD_XFER_INITIATE;
        new (&this->CMD_XFER_INITIATE) version6::CMD_XFER_INITIATE (obj);
    }
    explicit ServerOpcode(version6::CMD_XFER_DATA&& obj) {
        opcode = Opcode::CMD_XFER_DATA;
        new (&this->CMD_XFER_DATA) version6::CMD_XFER_DATA (obj);
    }
};
